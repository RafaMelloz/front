...
@Component({
  selector: 'modal-add-fixed-costs',
  imports: [
    DialogModule,
    InputNumberModule,
    InputTextModule,
  ...
  templateUrl: './add-fixed-costs.component.html',
})

export class AddFixedCostsComponent implements OnDestroy, OnInit {
  ...
  sendForm() {
    ...
    const sub = this.dataService.changeFixedCosts(this.dataTable).subscribe({
      next: (res) => {
        console.log(res);
      }
    });
