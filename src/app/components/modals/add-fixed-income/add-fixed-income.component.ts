...
@Component({
  selector: 'modal-add-fixed-income',
  imports: [
    DialogModule,
    InputNumberModule,
    InputTextModule,
  ...
  templateUrl: './add-fixed-income.component.html',
})

export class AddFixedIncomeComponent implements OnDestroy, OnInit {
  ...
  sendForm() {
    ...
    const sub = this.dataService.changeFixedIncome(this.dataTable).subscribe({
      next: (res) => {
        console.log(res);
      }
    });
