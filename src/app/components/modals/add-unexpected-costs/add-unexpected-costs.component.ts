...
@Component({
  selector: 'modal-add-unexpected-costs',
  imports: [
    DialogModule,
    InputNumberModule,
    InputTextModule,
  ...
  templateUrl: './add-unexpected-costs.component.html',
})

export class AddUnexpectedCostsComponent implements OnDestroy, OnInit {
  ...
  sendForm() {
    ...
    const sub = this.dataService.changeUnexpectedCosts(this.dataTable).subscribe({
      next: (res) => {
        console.log(res);
      }
    });
